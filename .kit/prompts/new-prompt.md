...
Create a new prompt template. The user wants a prompt that does: $@
...
2. **Choose a filename**: short, lowercase, hyphen-separated, descriptive (e.g. `code-audit.md`)
...
8. **Confirm** by showing the final file content and the slash command that activates it (e.g. `/code-audit runtime`)

## Guidelines

- Keep prompts action-oriented — they should tell the agent *what to do*, not just *what to think about*
...
- If the workflow already exists as a prompt here, extend it instead of duplicating — see `commit-push.md`, `code-audit.md`, `fix-issue.md`, `create-pr.md`, `file-issue.md`, `live-smoke.md`, `release-tagger.md`, `resolve-reviews.md`, `update-docs.md`
