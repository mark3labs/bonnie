...
func TestNoSandboxIsTheDefault(t *testing.T) {
	f, err := agentFactory("none", "", false, nil)
	if err != nil {
	...
	if f, err = agentFactory("", "", false, nil); err != nil || f == nil {
...
func TestDenyNetworkNeedsASandbox(t *testing.T) {
	_, err := agentFactory("none", "", true, nil)
	if err == nil {
		t.Fatal("want an error for --sandbox-deny-network without a sandbox")
	}
...
func TestDenyNetworkOnLocalIsRejected(t *testing.T) {
	...
	if _, err := agentFactory("local", "", true, nil); err == nil {
...
func TestUnavailableSandboxFailsAtStartup(t *testing.T) {
	_, err := agentFactory("microsandbox", "", false, []kit.Option{})
	if err == nil {
